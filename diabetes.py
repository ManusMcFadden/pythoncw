...
class Diabetes:
    def __init__(self, filepath) -> None:
        try:
            with open(filepath) as file: #filepath passed is the filename to be opened
                ...
                self.data = [row for row in reader] #make the rest of the rows into the data
    ...
    def web_summary(self, filepath: str) -> None:
        with open(filepath,"w") as html: #open the file that the html will be written to with the name as the filepath passed to it
            html.write("<html>\n")
            html.write("<table>\n")
            html.write("<tr>\n")
            ...
            while columncount < self.get_dimension()[1] - 2: #skipping class
                ...
                nyes = 0
                nno = 0
                while rowcount < self.get_dimension()[0] - 1: #iterate through each row
                    if self.data[rowcount][self.get_dimension()[1] - 1] == "Positive": #check last value for p/n
